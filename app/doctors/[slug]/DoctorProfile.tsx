...
export default function DoctorProfile({ doctor }: { doctor: DoctorRecord }) {
  ...
  return <main className="doctor-page">
    ...
    <section className="doctor-hero"><div className="doctor-photo"><img src={doctor.image_url} alt={name} loading="eager" decoding="async" /><span><BadgeCheck /> {copy.verified}</span></div><div className="doctor-intro"><div className="doctor-index">AKA / DOCTOR</div><h1>{name}</h1><p className="doctor-role">{role}</p><p className="doctor-bio">{bio}</p><div className="doctor-focus"><small>{copy.focus}</small><div>{focus.map((item) => <span key={item}><Check /> {item}</span>)}</div></div><div className="doctor-proof"><span><ShieldCheck /></span><div><strong>{copy.comfort}</strong><p>{copy.comfortText}</p></div></div></div></section>
