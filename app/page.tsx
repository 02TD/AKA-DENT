...
const copy = {
  ru: {
    ...
    servicesEye: 'Полный цикл', servicesTitle: 'Всё лечение — по одному понятному плану', servicesText: 'Цены и описания регулярно обновляются клиникой. Точную стоимость врач подтверждает после диагностики.', details: 'Подробнее',
    ...
    pricesEye: 'Открытый прайс', pricesTitle: 'Стоимость до визита, а не после', pricesText: 'Клиника регулярно обновляет актуальные стартовые цены.', from: 'от', priceNote: 'Цены указаны «от» и не являются публичной офертой. Точная стоимость определяется после осмотра.',
    ...
    footerText: 'Стоматология полного цикла в Караганде. Лечим так, чтобы не переделывать.', clinic: 'О клинике', support: 'Поддержка', disclaimer: 'Информация на сайте не заменяет консультацию врача', showAll: 'Показать все', showLess: 'Свернуть', mobileCall: 'Позвонить',
  },
  kk: {
    ...
    clinicAreas: 'Бір жерде барлық ем түрі:', areasLabel: 'Стоматологиялық ем бағыттары', areas: ['3D тексеру','Имплант орнату','Тіс протездеу','Тісті емдеу','Хирургиялық ем','Қызыл иекті емдеу'], serviceLabel: 'ҚЫЗМЕТ',
    ...
    servicesEye: 'Толық цикл', servicesTitle: 'Барлық ем — бір түсінікті жоспармен', servicesText: 'Бағалар мен сипаттамаларды клиника тұрақты түрде жаңартады. Нақты құнын дәрігер диагностикадан кейін растайды.', details: 'Толығырақ',
    ...
    pricesEye: 'Ашық прайс', pricesTitle: 'Құны қабылдауға дейін белгілі', pricesText: 'Клиника өзекті бастапқы бағаларды тұрақты түрде жаңартады.', from: 'бастап', priceNote: 'Бағалар бастапқы мәнде көрсетілген және жария оферта емес. Нақты құн тексеруден кейін анықталады.',
    ...
    footerText: 'Қарағандыдағы толық циклді стоматология. Қайта жасатпау үшін сапалы емдейміз.', clinic: 'Клиника туралы', support: 'Қолдау', disclaimer: 'Сайттағы ақпарат дәрігер кеңесін алмастырмайды', showAll: 'Барлығын көру', showLess: 'Жинау', mobileCall: 'Қоңырау',
  },
} as const;
const stats = [
      ...
      <footer className="border-t border-black/[.07] bg-[#F4F5F3] pt-20">
        <div className="site-container">
          ...
          <div className="flex flex-col gap-4 border-t border-black/[.07] py-7 text-[11px] text-[#8A9498] sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap gap-5"><span>{t.disclaimer}</span></div>
          </div>
        </div>
      </footer>
